...
//   120 CHK=0:FOR I=0 TO 4:FOR J=0 TO 9
//   130 PRINT V%(I,J);",";:CHK=CHK XOR V%(I,J)
//   140 NEXT J:PRINT:NEXT I
//   150 PRINT "CHECK:";HEX$(CHK)
// 9801:
//   100 N=0:CLEAR ,&H8000:PLAY ALLOC 255,255,255
//   110 DIM V%(4,9):VOICE COPY N,V%
//   120 CHK=0:FOR I=0 TO 4:FOR J=0 TO 9
//   130 PRINT V%(I,J);",";:CHK=CHK XOR V%(I,J)
//   140 NEXT J:PRINT:NEXT I
//   150 PRINT "CHECK:";HEX$(CHK)
const timbre_array_t ym2203_tone_table[NUM_TONES] = {
    ...
    ///////////////////////////////////////////////
    // NOTE: データは正しいはずなのに、音色が一致しない。エミュレーションが間違っているに違いない。
    // @62 "PERC 3" ウィンドチャイムのような音
    {
        {60, 15, 0, 0, 0, 0, 0, 0, 0, 0},
        {2, 19, 31, 9, 0, 102, 0, 8, 2, 0},
        {2, 19, 31, 9, 0, 117, 0, 5, 3, 0},
        {0, 19, 31, 9, 0, 101, 0, 7, 3, 0},
        {0, 19, 31, 9, 0, 107, 0, 5, 0, 0},
        // CHECK: 0023
    },
    // @63 "MUSIC BOX" オルゴールのような音
    {
        {59, 15, 2, 8, 6000, 0, 60, 0, 0, 0},
        {3, 14, 20, 8, 13, 122, 3, 6, 3, 10},
        {0, 12, 31, 8, 8, 92, 0, 7, 0, 5},
        {3, 12, 20, 8, 0, 110, 3, 15, -3, 0},
        {0, 18, 20, 9, 0, 127, 0, 4, 0, 15},
    }, // CHECK: E8A2
    // @64 "CELLO" チェロ
    {
        {58, 15, 2, 8, 3200, 36, 0, 1, 0, 0},
        {12, 21, 31, 8, 14, 99, 1, 1, 1, 0},
        {12, 15, 31, 5, 12, 97, 1, 7, 3, 0},
        {14, 15, 31, 7, 14, 87, 1, 0, 2, 0},
        {20, 0, 31, 7, 15, 127, 1, 0, 1, 0},
    }, // CHECK: 0CB6
    // @65 "LOW BRASS" 低域の金管楽器
    {
        {58, 15, 2, 10, 3000, 40, 0, 1, 0, 0},
        {17, 27, 31, 8, 13, 96, 0, 0, 1, 0},
        {16, 27, 31, 8, 12, 84, 0, 0, -3, 0},
        {16, 23, 31, 8, 12, 96, 0, 0, 3, 0},
        {14, 31, 19, 7, 12, 127, 0, 1, 0, 0},
    }, // CHECK: F46C
    // @66 "WW ENSMBL" 木管楽器のアンサンブル
    {
        {58, 15, 2, 12, 3200, 34, 0, 1, 0, 0},
        {18, 25, 29, 7, 14, 87, 2, 2, 0, 0},
        {16, 23, 31, 7, 14, 87, 1, 3, 2, 0},
        {10, 24, 31, 7, 13, 85, 0, 2, -2, 0},
        {13, 27, 31, 7, 13, 127, 1, 2, 1, 0},
    }, // CHECK: F346
    // @67 "AC GUITAR" アコースティック ギター
    {
        {58, 15, 2, 1, 2600, 20, 0, 1, 0, 0},
        {5, 22, 24, 11, 0, 94, 3, 1, -3, 0},
        {4, 28, 31, 11, 0, 76, 2, 5, 2, 0},
        {3, 28, 31, 11, 0, 84, 3, 3, 0, 0},
        {0, 26, 31, 8, 0, 127, 0, 1, 0, 0},
    }, // CHECK: F5C3
    // @68 "FLUTE/HARP" フルートとハープのユニゾン
    {
        {60, 15, 2, 8, 2600, 0, 60, 0, 0, 0},
        {3, 26, 22, 11, 0, 91, 3, 1, 0, 0},
        {0, 22, 31, 9, 0, 127, 0, 1, 0, 0},
        {12, 20, 31, 6, 14, 92, 1, 2, 2, 10},
        {16, 0, 31, 4, 15, 127, 0, 2, 0, 15},
    }, // CHECK: 0A20
    // @69 "FUNK PLUC" 打弦楽器の音
    {
        {59, 15, 4, 1, 3000, -50, 0, 1, 0, 0},
        {15, 23, 31, 9, 0, 79, 3, 11, 1, 0},
        {0, 19, 31, 9, 0, 72, 0, 6, 3, 0},
        {13, 19, 31, 9, 0, 115, 0, 3, 3, 0},
        {0, 0, 29, 8, 15, 124, 3, 1, 0, 0},
    }, // CHECK: F459
    // @70 "FUNK BASS" ファンキーなベースの音
    {
        {32, 15, 0, 0, 0, 0, 0, 0, 0, 0},
        {0, 24, 24, 6, 13, 111, 3, 8, 3, 0},
        {0, 25, 25, 6, 14, 67, 3, 7, 3, 0},
        {15, 22, 25, 6, 14, 115, 2, 0, 3, 0},
        {0, 25, 23, 6, 0, 127, 2, 1, 3, 0},
    }, // CHECK: 0002
    // @71 "SYN LEAD" リードの音
    {
        {58, 15, 2, 8, 3200, 42, 127, 1, 0, 0},
        {17, 23, 31, 9, 0, 87, 0, 12, 0, 0},
        {9, 23, 31, 9, 12, 77, 0, 8, 0, 0},
        {9, 23, 31, 9, 12, 97, 0, 2, 0, 1},
        {2, 0, 31, 7, 15, 127, 0, 2, 0, 0},
    }, // CHECK: 0CEF
    // @72 "METAL CLES" 金属的な音1
    {
        {56, 15, 5, 1, 2000, -40, 0, 1, 0, 0},
        {0, 0, 31, 12, 15, 106, 0, 14, -2, 0},
        {24, 0, 31, 11, 15, 114, 0, 3, 0, 0},
        {0, 24, 31, 10, 5, 90, 0, 11, 3, 0},
        {22, 24, 31, 10, 10, 127, 0, 4, 0, 0},
    }, // CHECK: 07FE
    // @73 "STAIN" 金属的な音2
    {
        {58, 15, 5, 8, 100, 127, 127, 1, 0, 0},
        {26, 26, 28, 12, 12, 102, 00, 0, 3, 5},
        {18, 0, 31, 10, 10, 110, 0, 1, 2, 5},
        {6, 26, 20, 8, 8, 98, 0, 10, -3, 5},
        {0, 0, 31, 11, 15, 127, 0, 0, 0, 0},
    }, // CHECK: FFB8
    // @74 "CUBIN GLASS" 氷がグラスの中を転がるような音
    {
        {60, 15, 1, 6, 6300, 0, 80, 0, 0, 0},
        {0, 12, 20, 10, 5, 110, 0, 15, 3, 0},
        {0, 12, 20, 10, 5, 127, 0, 14, 2, 0},
        {12, 20, 12, 8, 5, 92, 0, 15, 3, 10},
        {12, 20, 12, 8, 5, 127, 0, 8, 0, 15},
    }, // CHECK: 18CB
    // @75 "HUMAN" 人の声のような音
    {
        {58, 15, 2, 3, 3200, 50, 80, 1, 0, 0},
        {14, 0, 31, 5, 15, 82, 2, 2, 1, 1},
        {0, 10, 31, 5, 11, 104, 3, 10, -2, 2},
        {16, 0, 31, 5, 14, 78, 2, 1, 0, 4},
        {20, 0, 31, 6, 15, 127, 0, 2, 0, 0},
    }, // CHECK: F32A
    // @76 "WOOD BASS" ウッドベース
    {
        {42, 15, 0, 0, 0, 0, 0, 0, 0, 0},
        {0, 24, 20, 8, 0, 84, 3, 0, 3, 0},
        {5, 26, 31, 8, 8, 86, 2, 0, 2, 10},
        {5, 26, 31, 8, 5, 78, 3, 2, 2, 15},
        {3, 26, 31, 8, 8, 127, 0, 0, 0, 10},
    }, // CHECK: 0015
    // @77 "CHIMES" チャイム
    {
        {62, 15, 2, 0, 4000, 40, 14, 1, 0, 0},
        {0, 20, 20, 10, 0, 92, 0, 14, 1, 2},
        {16, 20, 20, 11, 0, 106, 0, 10, 3, 2},
        {10, 20, 20, 10, 0, 120, 0, 6, 2, 0},
        {0, 20, 20, 9, 0, 127, 0, 4, 0, 0},
    }, // CHECK: 0F9B
    // @78 "SPACY" 金属的な口笛の音
    {
        {5, 15, 2, 3, 3200, 50, 0, 1, 0, 0},
        {5, 26, 31, 13, 14, 55, 2, 6, 3, 3},
        {10, 28, 31, 12, 13, 90, 3, 15, 1, 5},
        {10, 28, 31, 10, 14, 90, 3, 4, 3, 1},
        {20, 0, 31, 7, 15, 127, 0, 4, 3, 2},
    }, // CHECK: 0CF9
    // @79 "OBOE/BCLAR" オーボエとバス クラリネットのユニゾン
    {
        {7, 15, 2, 7, 3300, 50, 30, 1, 0, 0},
        {18, 0, 31, 8, 15, 120, 3, 3, 3, 0},
        {20, 0, 31, 7, 15, 127, 2, 0, 3, 3},
        {25, 0, 31, 6, 15, 120, 3, 4, 2, 3},
        {20, 0, 31, 4, 15, 127, 2, 2, 1, 0},
    }, // CHECK: 0CC4
    // @80 "OLD STRING" 古いレコードのバイオリンのような音
    {
        {58, 15, 0, 0, 0, 0, 0, 0, 0, 0},
        {0, 19, 27, 5, 14, 95, 0, 12, 0, 0},
        {0, 29, 27, 9, 0, 70, 3, 15, 1, 0},
        {0, 19, 27, 9, 15, 97, 0, 1, 0, 0},
        {0, 26, 24, 8, 13, 127, 2, 3, -1, 0},
    }, // CHECK: FFC9
    // @81 "STEEL'S CRY" 金属的な鳴き声のような音
    {
        {60, 15, 0, 6, 6300, 30, 80, 10, 0, 0},
        {0, 12, 20, 10, 5, 110, 0, 8, 3, 3},
        {6, 12, 20, 10, 5, 120, 0, 6, 2, 5},
        {12, 20, 12, 8, 5, 100, 0, 8, 3, 13},
        {6, 20, 12, 8, 5, 127, 0, 8, 3, 15},
    }, // CHECK: 18E7
}; // ym2203_tone_table
...
{
    size_t i = 0;
    for (auto& item : ym2203_tone_table) {
        int16_t checksum = 0;
        for (auto& row : item) {
            for (auto& column : row) {
                checksum ^= column;
