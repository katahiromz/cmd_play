...
PC88 の `CMD PLAY`文を Windows でエミュレートするプログラムです。
...
`cmd_play`では現在、`#0`と`#2`以外は選択できません。*音源モード*を省略した場合は、`#2`が選択されます。各チャンネルの文字列は、MML (Music Macro Language)といい、次のような意味を持ちます。
