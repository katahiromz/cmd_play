...
古いパソコン PC-8801 の N88-BASIC という言語の `CMD PLAY` 文を現代の Windows パソコンでエミュレートするプログラムです。
...
`cmd_play`では現在、`#1`は選択できません。*音源モード*を省略した場合は、`#2`が選択されます。各チャンネルの文字列は、MML (Music Macro Language)といい、次のような意味を持ちます。
